...
public class ModularFeatureEditorDialog extends Dialog{
	...
	private List <  ?  > getChoicesofValues() {
		...
		if (res == null)
			eval.execute(Platform.getExtensionRegistry(), this.IdExtension, WTComponentsNewProjectNature.ID, eObject.eResource(), true, (EClass)feature.getEType());
		else
			eval.execute(Platform.getExtensionRegistry(), this.IdExtension, WTComponentsNewProjectNature.ID, res, false, (EClass)feature.getEType());
